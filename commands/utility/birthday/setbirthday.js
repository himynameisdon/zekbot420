...
function parseBirthdayDate(input) {
    ...
    return {
        ...
        year,
        hasYear: year !== null,
        display: year ? `${String(month).padStart(2, '0')}/${String(day).padStart(2, '0')}/${year}` : `${String(month).padStart(2, '0')}/${String(day).padStart(2, '0')}`,
    };
}
    ...
    async execute(message, args) {
        if (!args.length) {
            return message.reply(
                'Usage: `,setbirthday MM/DD/YYYY GMT-5` or `,setbirthday MM/DD GMT+1`\n' +
                'The year is optional. Run `,helptz` for a list of GMT timezones and example cities.'
            );
        }

        if (args.length < 2) {
            return message.reply(
                'Please include your timezone too.\n' +
                'Example: `,setbirthday 04/20 GMT-5`\n' +
                'Run `,helptz` for a list of GMT timezones and example cities.'
            );
        }

        const birthday = parseBirthdayDate(args[0]);
        if (!birthday) {
            return message.reply('Invalid birthday format. Use `MM/DD/YYYY` or `MM/DD`.');
        }
        ...
        return message.reply(
            `✅ Birthday saved as **${formatBirthdayLong(birthday)}** with timezone **${timezone.display}**.`
        );
    },
};