...
module.exports = {
    ...
    async execute(message) {
        ...
        if (mentionedUser) {
            const key = `${message.guild.id}:${mentionedUser.id}`;
            const birthday = birthdays[key];

            if (!birthday) {
                return message.reply(
                    `${mentionedUser} does not have a birthday set up in this server.`
                );
            }

            return message.reply(
                `${mentionedUser}'s birthday is on **${formatBirthdayDateWithYear(birthday)}**.`
            );
        }

        const serverBirthdays = Object.values(birthdays).filter(
            birthday => birthday.guildId === message.guild.id
        );

        const pages = buildBirthdayPages(serverBirthdays, message.guild);
        let currentPage = 0;
        ...
        collector.on('collect', async interaction => {
            ...
            if (interaction.customId === 'birthdays_prev') {
                currentPage = Math.max(currentPage - 1, 0);
            }

            if (interaction.customId === 'birthdays_next') {
