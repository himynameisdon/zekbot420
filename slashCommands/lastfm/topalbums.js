...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('topalbums')
        .setDescription('Show a Last.fm user’s top albums')
        .addStringOption((opt) =>
            opt
                .setName('username')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (!list.length) {
                return interaction.editReply(`No top albums found for **${username}** (${prettyPeriod}).`);
            }
            ...
            return interaction.editReply('Couldn’t fetch top albums right now—try again in a moment.');
        }
    }
};